// ════════════════════════════════════════════════════════════════════════════
// JUS DIGITALIS v2.6 — Named Counterparties & Dynamic Events Edition
// Changes from v2.5:
// 1. Removed Quick Demo tab
// 2. Replaced Scenario Mode with random market event system
// 3. Bot real names: Mehmet Yılmaz, Kerem Aslan, İbrahim Çelik
// 4. Contract types shown on cards (TBK references)
// 5. Player reputation score visible in header
// 6. Classic contracts probabilistic (not always failing)
// 7. Removed newsletter, free legal consult, more scenarios CTAs
// 8. After outcome → directly back to simulation (no CTA page)
// ════════════════════════════════════════════════════════════════════════════
...
const LS_KEY               = "jus_digitalis_v26";
...
// ─── BOT DATA ────────────────────────────────────────────────────────────────
// v2.6: real Turkish names + contract type references
const BOTS = [
  {
...
// ─── RANDOM EVENTS (v2.6 — replaces Scenario Mode) ───────────────────────────
const RANDOM_EVENTS = [
  {
    id:"foreign_capital",
    type:"positive",
    ...
    description:"Piyasaya yabancı yatırım girdi. Tüm sözleşme ödülleri %15 arttı ve başarı oranları iyileşti.",
    effect:{rewardBonus:0.15, successBonus:0.08},
  },
  {
    id:"trade_deal",
    type:"positive",
    emoji:"🤝",
    title:"Ticaret Anlaşması Bonusu",
    description:"Yeni uluslararası ticaret anlaşması imzalandı. Karşı tarafların güvenilirliği arttı.",
    effect:{successBonus:0.12, dominoBumpReduce:0.10},
  },
  {
    id:"pandemic_shock",
    type:"negative",
    emoji:"🦠",
    title:"Pandemi Şoku",
    description:"Salgın piyasaları sarstı. Tüm sözleşmelerde başarı oranları düştü, piyasa krizi aktif.",
    effect:{crashActive:true},
  },
  {
    id:"supply_chain",
    type:"negative",
    emoji:"⛓️",
    title:"Tedarik Zinciri Çöküşü",
    description:"Global tedarik zinciri aksadı. Botların teslim ihtimali azaldı, domino riski yükseldi.",
    effect:{dominoBump:0.15},
  },
];
...
function computeClassicDirectRate(bot, crashActive, eventEffect=null) {
  // Probability that a classic contract delivers without going to court
...
function computeAutopsy(method, bot, totalYears, won) {
  ...
  const courtFee  = computeCourtFee(basePrice);
...
const PlayerReputationDisplay = memo(function PlayerReputationDisplay({score}) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,.04)",border:`1px solid ${badge.color}44`,borderRadius:12,padding:"8px 14px"}}>
      ...
      <div>
        <div style={{fontFamily:"'Space Mono',monospace",color:badge.color,fontWeight:700,fontSize:13,letterSpacing:1}}>
          {score}/100
        </div>
...
// ─── BOT CARD (v2.6: shows contractType + contractRef) ────────────────────────
  ...
  return (
    <div>
      <div style={{textAlign:"center",marginBottom:24}}>
        ...
        {eventEffect?.rewardBonus && <div style={{background:"rgba(0,212,170,.08)",border:"1px solid rgba(0,212,170,.2)",color:"#68d391",borderRadius:8,padding:"8px 12px",marginTop:8,fontSize:12}}>✦ Aktif olay: ödüller %{Math.round(eventEffect.rewardBonus*100)} arttı</div>}
      </div>
...
// ─── FULL SIMULATION ──────────────────────────────────────────────────────────
function FullSimulation({abVariant, coins, setCoins, trustScores, setTrustScores, stats, setStats, capitalProtected, setCapitalProtected, legalRisk, setLegalRisk, sessionStart, sessionCount, eventEffect, onRoundComplete}) {
  ...
  // Apply event effects on mount when event changes
  useEffect(()=>{
  ...
  function handleMethodChoice(method) {
    ...
    if (method==="smart") {
      ...
      if (delivered) {
        ...
        const aut = computeAutopsy("classic",selectedBot,0,true);
        setAutopsy(aut);
        track("CONTRACT_OUTCOME",{botId:selectedBot.id,method:"classic",success:true,profitLoss:profit,durationSeconds:0,konkordatoTriggered:false,legalMode:"direct",yearsSpent:0});
        ...
        setPhase("classic_direct_outcome");
      } else {
  ...
  function handleAutopsyDone() {
    // v2.6: go directly back to simulation (no CTA page)
  ...
  if (phase==="select_bot") {
    return (
      <div style={cardStyle}>
        {coinAnim && (
          <div key={coinAnim.id} style={{position:"fixed",top:80,right:32,color:coinAnim.amount>0?"#00d4aa":"#ff4444",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:20,animation:"fadeUp .7s ease-out forwards",pointerEvents:"none",zIndex:500}}>
    ...
    return (
      <div style={cardStyle}>
        <div style={{textAlign:"center",marginBottom:28}}>
          ...
          <div style={{color:"#a0aec0",fontSize:12,marginBottom:4,fontFamily:"'Space Mono',monospace"}}>
            {selectedBot.contractType} · {selectedBot.contractRef}
          </div>
  ...
  // ── Phase: classic_direct_outcome (v2.6: bot delivered without lawsuit) ──
  if (phase==="classic_direct_outcome" && outcome && selectedBot) {
    return (
      <div style={cardStyle}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{fontSize:52}}>✅</div>
          <h3 style={{color:"#00d4aa",fontSize:22,marginTop:8,marginBottom:4}}>Teslimat Başarılı</h3>
          <p style={{color:"#718096",fontSize:13,marginTop:4}}>{selectedBot.name} yükümlülüğünü yerine getirdi.</p>
        </div>
        <div style={{background:"rgba(0,212,170,.06)",border:"1px solid rgba(0,212,170,.2)",borderRadius:12,padding:16,marginBottom:16,textAlign:"center"}}>
          <div style={{color:"#4a5568",fontSize:12,marginBottom:4}}>{selectedBot.contractType} · {selectedBot.contractRef}</div>
          <div style={{color:"#00d4aa",fontFamily:"'Space Mono',monospace",fontWeight:700,fontSize:20}}>+{outcome.profit} JC</div>
          <div style={{color:"#718096",fontSize:12,marginTop:2}}>Klasik sözleşme — doğrudan teslimat</div>
        </div>
        <LegalReceipt
          title="KLASİK SÖZLEŞME — BAŞARILI"
          color="#00d4aa"
          entries={[
            {label:"Ödeme (sözleşme bedeli)",value:`${selectedBot.basePrice} JC`,accent:"#718096"},
            {label:"Alınan",value:`${outcome.reward} JC`,accent:"#00d4aa",bold:true},
            {label:"Mahkeme harcı",value:"0 JC",accent:"#00d4aa",note:"Dava gerekmedi"},
            {label:"Bekleme süresi",value:"Anında",accent:"#00d4aa"},
          ]}
...
export default function App() {
  ...
  // v2.6: contract counter + random event system
  const [contractCount, setContractCount] = useState(0);
  const [activeEvent, setActiveEvent] = useState(null);
  const [eventEffect, setEventEffect] = useState(null);
  const nextEventAtRef = useRef(2 + Math.floor(Math.random()*2)); // trigger at 2 or 3
  ...
  function handleEventDismiss() {
    ...
    setEventEffect(activeEvent.effect);
    setActiveEvent(null);
    track("RANDOM_EVENT",{eventId:activeEvent.id,type:activeEvent.type});
  }
  ...
  // v2.6: player reputation derived from trust scores
  const playerReputation = computePlayerReputation(trustScores);

  // v2.6: only two tabs remain
  const MODULES = [
      ...
      <div style={{minHeight:"100vh",background:"#060a10",color:"#e2e8f0"}}>
        ...
        {activeEvent && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
            <div style={{
              background:"#0d1b2a",
              border:`2px solid ${activeEvent.type==="positive"?"rgba(0,212,170,.5)":"rgba(255,68,68,.5)"}`,
              ...
              <div style={{
                color:activeEvent.type==="positive"?"#00d4aa":"#ff4444",
                fontSize:11,letterSpacing:2,marginBottom:10,
                fontFamily:"'Space Mono',monospace",fontWeight:700,
              }}>
                {activeEvent.type==="positive"?"✦ POZİTİF PİYASA OLAYI":"⚠️ NEGATİF PİYASA OLAYI"}
              </div>
              ...
              <button onClick={handleEventDismiss} style={{
                padding:"14px 48px",
                background:activeEvent.type==="positive"?"linear-gradient(135deg,#00d4aa,#0099ff)":"linear-gradient(135deg,#ff4444,#ff6b35)",
        ...
        {eventEffect && !activeEvent && (
          <div style={{
            background:eventEffect.rewardBonus||eventEffect.successBonus||eventEffect.dominoBumpReduce
              ?"rgba(0,212,170,.08)":"rgba(255,68,68,.08)",
            borderBottom:`1px solid ${eventEffect.rewardBonus||eventEffect.successBonus||eventEffect.dominoBumpReduce
              ?"rgba(0,212,170,.2)":"rgba(255,68,68,.2)"}`,
            padding:"8px 24px",textAlign:"center",fontSize:12,
            color:eventEffect.rewardBonus||eventEffect.successBonus||eventEffect.dominoBumpReduce?"#68d391":"#fc8181",
          }}>
            {eventEffect.rewardBonus && `✦ Aktif: Yabancı Sermaye — ödüller +%${Math.round(eventEffect.rewardBonus*100)}`}
            {eventEffect.successBonus && `✦ Aktif: Ticaret Anlaşması — başarı +%${Math.round(eventEffect.successBonus*100)}`}
            {eventEffect.crashActive && "⚠️ Aktif: Pandemi Şoku — piyasa krizi devam ediyor"}
            {eventEffect.dominoBump && `⚠️ Aktif: Tedarik Zinciri Çöküşü — domino riski +%${Math.round(eventEffect.dominoBump*100)}`}
          </div>
        )}
        ...
        <header style={{borderBottom:"1px solid rgba(255,255,255,.06)",padding:"16px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:22,color:"#00d4aa",letterSpacing:2}}>
              JUS DIGITALIS
              <span style={{fontFamily:"'Space Mono',monospace",color:"#4a5568",fontSize:12,marginLeft:8,fontWeight:400}}>v2.6</span>
            </div>
            <div style={{color:"#4a5568",fontSize:11,marginTop:2}}>Named Counterparties & Dynamic Events</div>
          </div>

          <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <SocialProofWidget/>
            {/* v2.6: player reputation score */}
        ...
        <main style={{padding:"32px 24px",maxWidth:760,margin:"0 auto"}}>
          ...
          <div style={{textAlign:"center",marginTop:24,color:"#2d3748",fontSize:11,fontFamily:"'Space Mono',monospace"}}>
            JUS DIGITALIS v2.6 — Simülasyon amaçlıdır. Hukuki tavsiye niteliği taşımaz.
          </div>
        </main>
      </div>
    </JusErrorBoundary>
